# NOTE: The loops only work on a slower run speed

.text 
#TODO 4: ===============================================
# adjust the code to take multiple key presses until key F is pressed. 
# =======================================================
main: 
		addi	$a0, $0,0 		 
		jal 	init7SegDisp
...
Num2SegDisp:
		...
		lb	$t6, 0($t0)		# # load the target pattern as a copy
		lui 	$t0, 0xffff  		# load $t0 with base IO address
		...
		lb	$t1, 0($t0)    		# load the target pattern
		lui 	$t0, 0xffff  		# load $t0 with base IO address
		sb 	$t6, 0x11($t0) 		# send the target pattern to the 7-segment disp
		sb	$t1, 0x10($t0)		# send the target pattern to the 7-segment disp

		beq 	$a0, 0x71, exit		# check to see if "F" has been clicked. If yes, jump to exit
		#j	exit			# jump to exit
		
###################   Hexadecimal keyboard help  #######################
# Byte value at address 0xFFFF0012 : command row number of hexadecimal keyboard (bit 0 to 3) 
...
# 0xFFFF0014 byte value is composed of row number (4 bits) and column number (4 bits) 
 
# To determine the key, the mips program has to set  row bits one by one (i.e., send 1,2,4,8...)
# at 0xFFFF0012 and then reads 0xFFFF0014 to check if a key is pressed in this row
# =======================================================
...
getPressedKey: 
		...
		li 	$t3, 0			# clear register
		li  	$t4, 0			# clear register
		li 	$t5, 0			# clear register
		lui 	$t0, 0xffff   		# load $t0 with base IO address    
		
rowSearch:	addi 	$t4, $zero, 1     	# set $t4 to 1 to scan the first row
		...
		addi 	$t4, $zero, 2      	# set $t4 to 2 to scan the second row
		sb 	$t4, 0x12($t0)  	# command second row scanning at 0xFFFF0012
		lb  	$t2, 0x14($t0)  	# read  0xFFFF0014 for pressed keys 
		bne 	$t2, $zero, KeyPress 	# check any pressed keys [zero --> nothing pressed]	
		
		addi 	$t4, $zero, 4      	# set $t4 to 4 to scan the third row
		sb 	$t4, 0x12($t0)  	# command third row scanning at 0xFFFF0012
		lb  	$t2, 0x14($t0)  	# read  0xFFFF0014 for pressed keys 
		bne 	$t2, $zero, KeyPress	# check any pressed keys [zero --> nothing pressed]	
		
		addi 	$t4, $zero, 8     	# set $t4 to 8 to scan the fourth row
		sb 	$t4, 0x12($t0)  	# command fourth row scanning at 0xFFFF0012
		lb  	$t2, 0x14($t0)  	# read  0xFFFF0014 for pressed keys 
		bne 	$t2, $zero, KeyPress 	# check any pressed keys [zero --> nothing pressed]	
				
		j  	rowSearch  		# loop back to beginning of the row search
